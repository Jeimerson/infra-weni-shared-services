...
locals {
  ...
  workspace = local.env["workspaces"][(terraform.workspace == "default") ? "develop-us-east-1" : terraform.workspace]
  common    = local.env["common"] # tflint-ignore: terraform_unused_declarations
}
